...
// your code here (design the function to pass all tests)
function double($number=NULL) {

	$checkNumber = func_num_args();
	$checkValue = isset($number);
	$checkInteger = gettype($number);

	if ($checkNumber > 1) {
		return "You may only supply 1 value";
	} elseif ($checkValue != true) {
		return "You haven't supplied a value";
	} elseif ($checkInteger != 'integer') {
